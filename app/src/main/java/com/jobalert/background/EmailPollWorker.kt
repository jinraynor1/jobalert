package com.jobalert.background
...
import com.jobalert.domain.rule.RuleEngine
    ...
    override suspend fun doWork(): Result {
        ...
        for (account in accounts) {
            ...
            val fetchResult = withContext(Dispatchers.IO) {
                ImapClient.fetchNew(account, credential, app.settingsRepository.snippetMaxChars)
            }
                    ...
                    when {
                        ...
                        s.isWithinMinInterval -> Log.i(TAG, "INTERVALO MÍNIMO — overlay omitido")
                            ...
                            withContext(Dispatchers.Main) {
                                app.overlayManager.show(data.copy(ruleName = matchedRule.name, alertColor = matchedRule.alertColor))
                            }
                        }
                    }
                    newCount++
